  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm mb-8 text-center bg-gradient-to-b from-white to-gray-50/50">
        ...
        <p className="text-3xl md:text-5xl font-black text-gray-900 mb-8">
          ${totalHistoricalBalance.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          <span className="text-lg md:text-xl font-bold text-gray-400 ml-2">ARS</span>
        </p>
        
        <div className="flex overflow-x-auto md:flex-wrap justify-start md:justify-center gap-4 pb-4 md:pb-0 scrollbar-hide">
          {historicalAccounts.map(account => (
            <div key={account.id} className="bg-white px-4 py-3 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3 min-w-[160px] flex-shrink-0">
              <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center">
                {getAccountIcon(account.type)}
              </div>
              <div className="text-left">
                <p className="text-[10px] font-bold text-gray-400 uppercase truncate max-w-[100px]">{account.name}</p>
